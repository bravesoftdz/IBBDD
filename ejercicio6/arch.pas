...
interface
  ...
  //De los archivos:
	...
	procedure Respaldar (var A:apersonas; nombre:String);                                      //Crea una nueva versiÛn del archivo sin espacios libres;
...
implementation
	...
	begin
		...
		begin
			read(A.ra.arch,A.p);
			A.ra.pos:= A.ra.pos +1;
			if (A.p.valido)then //solo escribe en el .txt si es un registro v√°lido
			begin
	...
	begin
		...
		begin
			Seek(A.lib, FileSize(A.lib)-1); //#Hay que ver si FileSize(A.lib) lleva a la posicion de EoF o a la del √∫ltimo elemento
			read(A.lib, pos);
	...
	begin
		...
		A.ra.arch:= nuevo;   //# Esto no s√© si se puede hacer
	end;
	procedure Cerrar(var A:apersonas);
	begin
		close(A.ra.arch);
